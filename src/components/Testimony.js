...
function Testimony({ className }) {
	const Testimony = [
		{
			content:
				"Canalit Consultant handled my visiting Visa and renewal. The service was extremely great, and I was very satisfied. They went the extra miles to put everything in place while staying efficient. They did an excellent job for me.",

			name: "Beatrice Akhaine",
		},
		{
			content:
						...
						<Slider auto={12000} hasArrows>
							{Testimony.map((each_testimony) => {
								return (
									...
									>
										<p>
											{each_testimony.content}{" "}
											<span>{each_testimony.name}</span>
										</p>
									</div>
								);
							})}
